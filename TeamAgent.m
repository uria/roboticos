...
function action = QLambda(S, t, player)
    ...
    if t > 1
        r = reward(S);
        delta = r - Q(player, a(player));
    end
     
    F = get_features_for_player(player, S);
    for i = 1:numActions        
        theta_index = numBinFeatures*(i - 1) + 1;
        Q(player, i) = theta(theta_index:theta_index + numBinFeatures-1) * F';        
    end
    
    [action, Qmax_a] = argmax(Q(player, :));
    
    if t > 1
        delta = delta + gamma * Qmax_a;
        theta = theta + alpha * delta * e(player, :);
    end
    
    if rand(1) > epsilon
        e(player, :) = gamma * lambda * e(player, :);
    else
        action = randi(numActions);
        e(player, :) = zeros(1, numBinFeatures * numActions);
    end
    
    e_index = numBinFeatures * (action - 1) + 1;
    e(player, e_index:e_index+numBinFeatures-1) = e(player, e_index:e_index+numBinFeatures-1) + F;
end

function [index, value] = argmax(array)
    index = 1;
    value = array(1);
